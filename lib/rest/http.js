  ...
  Promise
    .try(() => {
      ...
      let pn    = path.normalize(parsed_url.pathname)
        , parts =  _.chain(pn.split(path.sep)).tail().compact().value()
        ;
      ...
      let host      = parts[0]
        ...
        , bearer    = token? `Bearer ${token}` : undefined
        ;
  ...
  Promise
    ...
    .then((user) => {
      ...
      codedb_sdk
        ...
        .then((model) => {
          res.render('git/playground', { model, parsed, breadboard_url, user });
        })
        .catch((err) => {
          next(err, req, res);
        });
